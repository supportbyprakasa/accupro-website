...
(function () {
  ...
  if (finder) {
    ...
    finder.addEventListener('submit', function (e) {
      e.preventDefault();
      /* The static build's option values are paths relative to data-base. The
         WordPress theme renders the same markup but with real permalinks, so a
         category option can carry its own data-url and the whole-catalogue
         fallback comes from data-all. An absolute value is used as-is;
         anything else is still resolved against data-base. */
      var opt = catSel.options[catSel.selectedIndex];
      var go = svcSel.value ||
        (opt && opt.dataset.url) ||
        (catSel.value ? 'services/' + catSel.value + '.html' : (finder.dataset.all || 'services.html'));
      var absolute = /^[a-z]+:/i.test(go) || go.charAt(0) === '/';
      window.location.href = absolute ? go : finder.dataset.base + go;
    });
  }
